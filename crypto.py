...
libssl.EC_GROUP_get_order.argtypes = [ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p]
libssl.EC_GROUP_get_order.restype = ctypes.c_int
...
libssl.i2d_ECDSA_SIG.argtypes = [ctypes.c_void_p, ctypes.c_void_p]
libssl.i2d_ECDSA_SIG.restype = ctypes.c_int

libssl.ECDSA_do_sign.argtypes = [ctypes.c_void_p, ctypes.c_int, ctypes.c_void_p]
libssl.ECDSA_do_sign.restype = ctypes.POINTER(ECDSA_SIG)
...
libssl.BN_cmp.argtypes = [ctypes.c_void_p, ctypes.c_void_p]
libssl.BN_cmp.restype = ctypes.c_int
...
libssl.BN_CTX_start.argtypes = [ctypes.c_void_p]
libssl.BN_CTX_start.restype = None

libssl.BN_CTX_end.argtypes = [ctypes.c_void_p]
libssl.BN_CTX_end.restype = None

libssl.BN_CTX_get.argtypes = [ctypes.c_void_p]
libssl.BN_CTX_get.restype = ctypes.c_void_p

libssl.BN_num_bits.argtypes = [ctypes.c_void_p]
libssl.BN_num_bits.restype = ctypes.c_int

libssl.BN_rshift1.argtypes = [ctypes.c_void_p, ctypes.c_void_p]
libssl.BN_rshift1.restype = ctypes.c_int

libssl.BN_sub.argtypes = [ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p]
libssl.BN_sub.restype = ctypes.c_int
...
class Key:
	...
	def sign(self, data):
		...
		# https://github.com/bitcoin/bitcoin/blob/e0e14e43d9586409e42919f6cb955540134cda2a/src/key.cpp

		sig = libssl.ECDSA_do_sign(ctypes.byref(b_data), len(data), self.keyData)
		if not sig:
			raise Exception("ECDSA_do_sign failed")

		#print 'Before transform:'
		#print 'r: ', bignum2str(sig.contents.r).encode('hex')
		#print 's: ', bignum2str(sig.contents.s).encode('hex')

		ctx = ctypes.c_void_p(libssl.BN_CTX_new())
		libssl.BN_CTX_start(ctx)

		group = ctypes.c_void_p(libssl.EC_KEY_get0_group(self.keyData))
		order = ctypes.c_void_p(libssl.BN_CTX_get(ctx))
		halforder = ctypes.c_void_p(libssl.BN_CTX_get(ctx))
		libssl.EC_GROUP_get_order(group, order, ctx)
		libssl.BN_rshift1(halforder, order)
		if libssl.BN_cmp(sig.contents.s, halforder) > 0:
			# enforce low S values, by negating the value (modulo the order) if above order/2.
			libssl.BN_sub(sig.contents.s, order, sig.contents.s)

		libssl.BN_CTX_end(ctx)
		libssl.BN_CTX_free(ctx)
		...
		#print 'Size 1: ', size.value
		b_sig = ctypes.create_string_buffer(size.value) # Make sure it is big enough

		pos = ctypes.pointer(b_sig)
		size = ctypes.c_int(libssl.i2d_ECDSA_SIG(sig, ctypes.byref(pos)))
		#print 'Size 2: ', size.value
		libssl.ECDSA_SIG_free(sig)
